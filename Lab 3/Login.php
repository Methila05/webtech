...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
  if (empty($_POST["userName"])) {
    $userNameErr = "Requires user name";
    $verify = 0;
  } else {
    $userName = test_input($_POST["userName"]);
    // check if name only contains letters and whitespace
    if (!preg_match("/^[-.a-zA-Z0-9_]*$/",$userName)) {
      $userNameErr = "Only numbers, letters, period and dash is allowed";
      $userName = "";
      $verify = 0;
    }
    else {
    	...
    	if (!preg_match('#[@$%]{1}#', $password)) {
     	...
     	$verify = 0;
    		...
    		if (strlen($password) < 8 ){
    			$passwordErr = "Password must not be less than eight characters";
    			$password = "";
    			$verify = 0;
    		}
    		else{
  ...
  if ($verify == 1){
	...
	echo "<h2> Enter a valid user name and password</h2>";
}
