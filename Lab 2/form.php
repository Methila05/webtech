...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
  if (empty($_POST["name"])) {
    ...
    if (!preg_match("/^[a-zA-Z-. ]*$/",$name)) {
      $nameErr = "Can contain a-z,A-Z, period, dash and white space only";
      $name = "";
    }
    else {
      if (str_word_count($name) < 2){
        $nameErr = " Must have at least 2 words";
        $name = "";
      }
    }
  }
  ...
  if ($degreeCount < 2) {
    $degreeErr = "Must select at least 2 of the boxes.";
...
<form method="post" action="<?php echo htmlspecialchars($_SERVER["PHP_SELF"]);?>  
  ...
  <span style = "display:inline-block; width:200px;text-align:left;"><b>BLOOD GROUP:</b></span> <br>
